...
describe("tokenizeInline", () => {
  it("returns a single text token for plain text", () => {
    expect(tokenizeInline("just words")).toEqual([{ type: "text", content: "just words" }]);
  });
...
describe("parseBlocks", () => {
  it("splits headings, quotes, lists, code, hr, paragraphs", () => {
    ...
    expect(blocks[1]).toEqual({ type: "paragraph", text: "A paragraph line one line two" });
    expect(blocks[2]).toEqual({ type: "quote", text: "a quote" });
    expect(blocks[3]).toEqual({ type: "list", items: ["one", "two"], ordered: false });
  ...
  it("detects ordered lists", () => {
    ...
    expect(blocks[0]).toEqual({ type: "list", items: ["first", "second"], ordered: true });
  });
});
